...
<html>
	<head>
		...
		<header>
			...
			<form method="post" action="prodaja_ulaznica.php" class="izborJezika" name="izborJezika"
	...
	<body>
		...
		<div class="koso">
			<?php
				$query = "SELECT * FROM jezik WHERE naziv='gdjeSmo'";
				$result = mysqli_query($con, $query);
				$row = mysqli_fetch_array($result);
				if(!empty($_POST))	
					echo $row[$_POST['jezik']];
		...
		<div id="onama">
			<?php
				$query = "SELECT * FROM jezik WHERE naziv='adresa'";
				$result = mysqli_query($con, $query);
				$row = mysqli_fetch_array($result);
				if(!empty($_POST))	
					echo $row[$_POST['jezik']];
		...
		<div class="koso">
			<?php
				$query = "SELECT * FROM jezik WHERE naziv='radnoVrijeme'";
				$result = mysqli_query($con, $query);
				$row = mysqli_fetch_array($result);
				if(!empty($_POST))	
					echo $row[$_POST['jezik']];
		...
		<div id="onama">
			<?php
				$query = "SELECT * FROM jezik WHERE naziv='radnoVrijemeNormalno'";
				$result = mysqli_query($con, $query);
				$row = mysqli_fetch_array($result);
				if(!empty($_POST))	
					echo $row[$_POST['jezik']];
		...
		<div id="onama">
			<?php
				$query = "SELECT * FROM jezik WHERE naziv='radnoVrijemePosebno'";
				$result = mysqli_query($con, $query);
				$row = mysqli_fetch_array($result);
				if(!empty($_POST))	
					echo $row[$_POST['jezik']];
		...
		<div id="onama">
			<?php
				$query = "SELECT * FROM jezik WHERE naziv='telefon'";
				$result = mysqli_query($con, $query);
				$row = mysqli_fetch_array($result);
				if(!empty($_POST))	
					echo $row[$_POST['jezik']];
		...
		<script>
  			function initMap() {
    			...
    			var map = new google.maps.Map(mapDiv, {
      				center: {lat: 45.815011, lng: 15.981919},
     				zoom: 11
    			});
    			var marker = new google.maps.Marker({
    				position: {lat: 45.769262, lng: 16.001898},
    				map: map
  				});
  			}
		</script>
